...
Wir wollen in diesem Abschnitt uns mit der longue durée der Demokratisierung beschäftigen. Das bedeutet eine ideengeschichtliche Rekonstruktion des Aufstiegs des demokratischen Gedankes (Theoretische Entwicklung). Danach geht es um den tatsächlichen Siegeszug dieser Herrschaftsform (Empirische Entwicklung). Um dieses zu leisten, müssen wir wuns fragen, wie man Demokratie misst. Hierzu diskutieren wir verschiedene Ansätze, und in diesem Zusammenhang Fragen der Reliabilität und Validität. 


## Staaten und Demokratie

* In VL2 haben wir uns mit dem Staat und dessen Existenz befasst
* Dabei war es aber relativ egal, wie diese Staaten letztlich regiert werden
* Der Staat hat einen Vorteil in der Ausübung von Gewalt *sowohl* in Demokratien *als auch* in Diktaturen
* Wie hat sich die Demokratie *theoretisch* als auch *empirisch* entwickelt?

## Theoretische Entwicklung

* Demokratie heutzutage quasi ausschließlich positiv konnotiert
* Selbst die stärksten Autokratien "schmücken" sich mit der Bezeichnung als Demokratie
  * Bspw.: Demokratische Volksrepublik Korea, DDR, ...
* Diese *positive* Konnotation ist aber ein relativ junges Phänomen
* Seit der Antike war Demokratie als Herrschaftsform verpönt
  * Dies lag weniger an einer Ablehnung der heutigen Form von Demokratie
...
```{r Demokratie, echo=FALSE, warning=FALSE, message=FALSE}
library(tidyverse)
...
```{r Przeworski, collapse=TRUE, warning=FALSE, message=FALSE, eval = F, size = "tiny"}
library(tidyverse)
library(democracyData) # nachdem das Paket installiert wurde
# remotes::install_github("xmarquez/democracyData")
# das Paket lässt sich nur über Github installieren, als Voraussetzung braucht man 

# dann einfach PIPE aufrufen
# PIPE
...
```{r Przeworskia, warning=FALSE, message=FALSE, echo = F, size = "tiny"}
...
![](particify_demokratie.png){width=10cm}
